'use strict';
// Minimal WebSocket client (RFC 6455) in pure Node (net + crypto).
// Used only when the runtime has no global WebSocket (Node >= 20.10).
// Implements exactly what the plugin needs: text messages, ping/pong, close.
...
class WebSocketClient {
  constructor(url) {
    ...
    if (!m) {
      this._fail(new Error('Invalid ws URL: ' + url));
      return;
    }
    this._host = m[1];
  ...
  _onData(chunk) {
    ...
    // Frame parsing (server frames are NOT masked)
    while (this._buffer.length >= 2) {
      ...
      // opcode 0x0 (continuation) and 0x2 (binary) are ignored — the server only sends text
    }
  }
