...
function request(opts) {
  return new Promise((resolve, reject) => {
    ...
    req.on('error', reject);
    req.setTimeout(opts.timeout || 20000, () => req.destroy(new Error('Request timeout')));
    if (opts.body) req.write(opts.body);
    req.end();
  });
}

// Request to the token endpoint (accounts.spotify.com/api/token)
...
class SpotifyApi {
  ...
  async api(method, path, opts, attempt) {
    ...
    if (!token) throw new AuthError('No token — sign in to your account');
    ...
    // 401 → try to refresh the token and retry once
    if (res.status === 401 && attempt === 0) {
      await this.auth.refresh();
      return this.api(method, path, opts, 1);
    }
    // 429 → wait and retry (at most two retries)
  ...
  // 0..100; if deviceId is omitted — the active device's volume
  ...
  // Fallback to the legacy /v1/me/library endpoints (like the official MiraBox plugin):
  // if the modern /v1/me/tracks returns 403, try the legacy variant with uris.
