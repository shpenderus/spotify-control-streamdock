'use strict';
// Average cover luminance (0..255) — used to pick the text color over it (black/white).
// Spotify has no public "cover color" API, so we compute luminance ourselves from the image buffer.
// Supports PNG (via built-in zlib) and baseline-sequential JPEG (we only unpack
// DC coefficients — their average gives the mean luminance without full decoding).
// Any error → null (text stays white, as before).

const zlib = require('zlib');

// The bottom part of the cover (~40%): that's where the text sits, so we sample it.
...
function averageLuminance(buffer) {
  ...
  } catch (e) { /* couldn't parse — return null */ }
  return null;
}
...
function pngLuminance(buf) {
  ...
  else return null; // palette (3) and others — skip
  ...
  for (let y = 0; y < height; y++) {
    ...
    off += stride;
    // Unfilter (for filters other than None)
    if (filter !== 0) {
...
class BitReader {
    ...
    if (this.bitCnt === 0) {
      ...
      if (b === 0xFF) {
        ...
        if (this.buf[this.pos] === 0x00) {
          ...
          // 0xFF D0-D7 — restart marker; return -1 as a signal
          return -1;
        }
      }
      this.bitBuf = b;
  ...
  // Byte-align and skip the restart marker RSTn (if present)
  alignAndSkipRestart() {
    this.bitCnt = 0;
...
// Reads the "size" (bit count) and the value itself; for AC coefficients the value
// isn't needed, but the bits must be consumed so the bitstream stays in sync.
...
function jpegLuminance(buf) {
  ...
  while (pos + 4 <= buf.length) {
    ...
    const body = pos + 4;
    if (marker >= 0xD0 && marker <= 0xD7) { pos += 2; continue; } // RST outside a scan — shouldn't happen
    if (marker === 0xDA) { // SOS: start of entropy data
      scanComps = { body, len };
      break;
    }
    ...
    if (marker === 0xDB) { // DQT
      ...
      if (marker !== 0xC0) return null; // only baseline supported (SOF0)
  ...
  // The Y component is the one with the largest sampling factor (usually the first)
  ...
  let sum = 0;
  let count = 0;
  const startRow = Math.floor(mcusY * BOTTOM_RATIO); // only sample the bottom MCU rows
      ...
      for (const sc of compOrder) {
        ...
        for (let b = 0; b < blocksPerMcu; b++) {
          ...
          if (comp.id === yComp.id && my >= startRow) {
            // average block luminance = 128 + F(0,0)/8, where F(0,0) = dc * q[0]
            sum += 128 + (dc * q[0]) / 8;
            count++;
          }
          // AC: skip so the bitstream stays in sync
          for (let k = 1; k < 64; k++) {
            const sym = decodeHuff(br, huffAC[sc.tac]);
            if (sym === 0) break; // EOB
            if (sym === 0xF0) { k += 15; continue; } // 16 zeros
            const run = sym >> 4;
            const size = sym & 0x0F;
            k += run;
