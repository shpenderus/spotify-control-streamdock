'use strict';
// Рендер всех состояний кнопок в виде SVG data-URL (StreamDock принимает data:image/svg+xml;base64 в setImage).
...
const LIGHT = '#B3B3B3';
const WHITE = '#FFFFFF';
const LOAD = '#8a8a8a'; // лайк ещё не известен (загружается)
const SIZE = 72;
const FONT_MIN = 6;
const FONT_MAX = 30;

// Авто-контраст: если низ обложки светлый — тёмный текст, тёмный — белый.
// Порог ~150 (из 255). luma == null (не посчитана) → белый текст, как раньше.
const LUMA_THRESHOLD = 150;
const INK = '#101010';   // почти чёрный
const INK_SOFT = '#3d3d3d'; // тёмно-серый для исполнителя на светлой обложке

// ВАЖНО: StreamDock принимает в setImage только готовый data-URL,
// сырую XML-строку SVG он игнорирует (кнопка остаётся с прежней картинкой).
...
// Эффект нажатия: кнопка «вдавливается» — контент временно уменьшается по центру.
// Меняется форма, а не цвет (заливка поверх делала тёмные кнопки «чёрным экраном»).
...
// Тонкий зелёный прогресс-бар снизу кнопки (как в плеере Spotify). Опциональный
// (settings.progressBar). Полоса — серый трек, поверх — зелёная заполненная часть.
...
// Тёмный «чип» с глифом play/pause.
// Глиф в зелёном цвете остальных кнопок (ранее был белым).
...
// Бегущая строка: текст не влезает — плавно прокручивается влево (две копии для бесшовного цикла).
// Скорость фиксированная: 14 px/с (2 кадра/с по 7px) — проверенный темп для StreamDock.
...
// примерная ширина символа Arial: ~0.52 * fontSize; доступное поле ~62px
function fitChars(size) {
  return Math.max(4, Math.floor(62 / (size * 0.52)));
}

// Собирает строки текста в блок. Возвращает { html, height }
...
// Авто-контраст (autoContrast): весь текст становится чёрным/белым по яркости низа обложки,
// чтобы не сливаться. Выключен — классические цвета (белый/серый, время в timeColor).
function buildLines(track, settings, progressMs, size, luma) {
  ...
  const dark = auto && luma > LUMA_THRESHOLD; // светлая обложка → тёмный текст
  ...
  // Цвет времени настраивается (timeColor), по умолчанию — зелёный Spotify.
  // При авто-контрасте время тоже чёрное/белое (свой цвет игнорируется).
  if (settings.time) {
...
// Нужна ли бегущая строка на этой кнопке (текст не влезает)
...
/* ---------- Глифы новых кнопок (плейлист / устройство / таймер сна) ---------- */

// Плейлист: строки списка + треугольник play
...
// Кнопка с подписью: иконка сверху, текст снизу (плейлист).
// Длинный текст прокручивается бегущей строкой, как на остальных кнопках.
function renderLabelButton(opts) {
  ...
  // Фаза бегущей строки (2 кадра/с, как на остальных кнопках) попадает в картинку
  // ТОЛЬКО когда текст не влезает: иначе SVG стабилен, и дедупликация setIcon
  // не шлёт картинки каждые 100 мс (поток картинок забивал устройство).
...
/* ---------- Статические иконки (зеркалят imgs/*.svg) ---------- */
...
function renderPlayPause(opts) {
  ...
  // Иконка play/pause на кнопке — опциональная (showIcon, по умолчанию выключена)
  ...
  if (lines.length) {
    ...
    if (hasCover) {
      ...
      // текст прямо на обложке, без тёмной плашки
  ...
  // Тонкий зелёный прогресс-бар снизу (опция «Прогресс-бар»)
...
function renderInfo(opts) {
  ...
  if (cover && settings.cover !== false) {
    ...
    // без обложки — текст по центру на тёмном фоне
    inner = bgRect();
